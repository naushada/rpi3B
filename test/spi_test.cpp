...
TEST_F(SPITest, CS_Field_SetClrGet) {
    spi().set_cs(BCM2837::SPIRegistersAddress::ControlStatus::TA, 1);
    EXPECT_EQ(spi().get_cs(BCM2837::SPIRegistersAddress::ControlStatus::TA), 1U);
    spi().clr_cs(BCM2837::SPIRegistersAddress::ControlStatus::TA);
    EXPECT_EQ(spi().get_cs(BCM2837::SPIRegistersAddress::ControlStatus::TA), 0U);
}
...
TEST_F(SPITest, Mode_Sets_CPOL_CPHA) {
    spi().mode(1, 1);
    EXPECT_EQ(spi().get_cs(BCM2837::SPIRegistersAddress::ControlStatus::CPOL), 1U);
    EXPECT_EQ(spi().get_cs(BCM2837::SPIRegistersAddress::ControlStatus::CPHA), 1U);
}
...
TEST_F(SPITest, Clear_FIFO_SetsBothBits) {
    spi().clear_fifo();
    EXPECT_EQ(spi().get_cs(BCM2837::SPIRegistersAddress::ControlStatus::CLEAR), 0b11U);
}

TEST_F(SPITest, High_Bit_Field_CSPOL2) {
    /* CSPOL2 lives at bit23 -- exercises high-bit field math. */
    spi().set_cs(BCM2837::SPIRegistersAddress::ControlStatus::CSPOL2, 1);
    EXPECT_EQ(spi().get_cs(BCM2837::SPIRegistersAddress::ControlStatus::CSPOL2), 1U);
